...
//   const getSubtotal = (bag) => {
//     const n = Number(bag.numberOfBags) || 0;
//     const p = Number(bag.pricePerBag) || 0;
//     return n * p;
//   };
...
//   const handlePreview = () => {
//     if (!isValid) {
//       showToast('Please fill customer name and all bag details', 'error');
//       return;
//     }
//     setShowModal(true);
//   };
...
//     setLoading(true);
//     try {
//       const res = await fetch('/api/debit-sale', {
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify({ customerName, bags, grandTotal })
//       });
...
//         {bags.map((bag, index) => (
//           <div key={index} className="bg-white rounded-2xl shadow-md border border-border p-5 relative hover:shadow-lg transition-shadow">
//             <div className="flex items-center justify-between mb-4">
//               <span className="text-sm font-bold text-primary-dark bg-primary/10 px-3 py-1 rounded-full">
//                 Bag #{index + 1}
//               </span>
//               {bags.length > 1 && (
//                 <button
...
//                   type="number"
//                   min="1"
//                   value={bag.numberOfBags}
//                   onChange={e => updateBag(index, 'numberOfBags', e.target.value)}
//                   placeholder="0"
//                   className="w-full px-4 py-3 rounded-xl border-2 border-border bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm font-medium"
//                 />
//               </div>
...
//                   type="number"
//                   min="1"
//                   value={bag.pricePerBag}
//                   onChange={e => updateBag(index, 'pricePerBag', e.target.value)}
//                   placeholder="0"
//                   className="w-full px-4 py-3 rounded-xl border-2 border-border bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm font-medium"
//                 />
//               </div>
...
//   const isValid =
//     customerName.trim() &&
//     bags.every(b => b.bagName.trim() && Number(b.numberOfBags) > 0 && Number(b.pricePerBag) > 0);
...
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify({
//           date: getTodayDate(),        // YYYY-MM-DD for database
...
//               <div>
//                 <label className="block text-sm font-semibold text-primary-dark mb-1.5">
//                   Price per Bag (₹) <span className="text-danger">*</span>
//                 </label>
//                 <input
//                   type="number"
//                   min="1"
...
//                     <td className="py-2 px-2 text-center">{bag.numberOfBags}</td>
//                     <td className="py-2 px-2 text-right">₹{Number(bag.pricePerBag).toLocaleString('en-IN')}</td>
//                     <td className="py-2 px-2 text-right font-semibold text-primary-dark">
//                       ₹{getSubtotal(bag).toLocaleString('en-IN')}
//                     </td>
//                   </tr>
//                 ))}
//               </tbody>
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        date:          todayISO,
        customer_name: customerName.trim(),
        customerName:  customerName.trim(),  // send both for compatibility
        bags,
        grandTotal,
        amount:        grandTotal             // send both for compatibility
      };

      console.log('Sending debit payload:', payload);
      ...
      console.error('Debit save error:', err);
      showToast(err.message || 'Failed to save. Please try again.', 'error');
    } finally {
      setLoading(false);
    }
        ...
        {bags.map((bag, index) => (
          <div key={index} className="bg-white rounded-2xl shadow-md border border-border p-5 hover:shadow-lg transition-shadow">
            <div className="flex items-center justify-between mb-4">
              ...
              {bags.length > 1 && (
                <button onClick={() => removeBag(index)} className="text-xs font-semibold text-danger hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors">
                  ✕ Remove
                </button>
              )}
            </div>
            <div className="grid sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-semibold text-primary-dark mb-1.5">Bag Name <span className="text-danger">*</span></label>
                <input type="text" value={bag.bagName} onChange={e => updateBag(index, 'bagName', e.target.value)}
                  placeholder="Enter bag name"
                  className="w-full px-4 py-3 rounded-xl border-2 border-border bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm font-medium" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-primary-dark mb-1.5">No. of Bags <span className="text-danger">*</span></label>
                <input type="number" min="1" value={bag.numberOfBags} onChange={e => updateBag(index, 'numberOfBags', e.target.value)}
                  placeholder="0"
                  className="w-full px-4 py-3 rounded-xl border-2 border-border bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm font-medium" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-primary-dark mb-1.5">Price per Bag (₹) <span className="text-danger">*</span></label>
                <input type="number" min="1" value={bag.pricePerBag} onChange={e => updateBag(index, 'pricePerBag', e.target.value)}
                  placeholder="0"
                  className="w-full px-4 py-3 rounded-xl border-2 border-border bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm font-medium" />
              </div>
            </div>
