...
//   const handlePreview = () => {
//     if (!isValid) {
//       showToast('Please fill all bag details correctly', 'error');
//       return;
//     }
//     setShowModal(true);
//   };
...
//     setLoading(true);
//     try {
//       // Trigger Webhook with detailed data
//       const webhookData = {
//         event: 'CASH_SALE_CREATED',
...
//       fetch('https://n8n.srv1202847.hstgr.cloud/webhook/7c04b69e-af9d-4e1d-b052-43959d34a90b', {
//         method: 'POST',
...
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify({ bags, grandTotal })
//       });
...
export default function CashSale() {
  ...
  const isValid = bags.every(
    b => b.bagName.trim() && Number(b.numberOfBags) > 0 && Number(b.pricePerBag) > 0
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Save to database — send all required fields
      const res = await fetch('/api/cash-sale', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <div className="mb-4 text-sm text-text-secondary text-center">
        📅 Recording for: <span className="font-bold text-primary-dark">{new Date().toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
      </div>
